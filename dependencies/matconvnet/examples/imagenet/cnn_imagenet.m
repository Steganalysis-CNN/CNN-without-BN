  ...
  switch opts.modelType
    case 'resnet-50'
      net = cnn_imagenet_init_resnet('averageImage', rgbMean, ...
                                     ...
                                     'classNames', imdb.imdb.classes.name, ...
                                     'classDescriptions', imdb.imdb.classes.description) ;
...
% -------------------------------------------------------------------------
images = strcat([imdb.imageDir filesep], imdb.imdb.images.name(batch)) ;
if ~isempty(batch) && imdb.imdb.images.set(batch(1)) == 1
  phase = 'train' ;
else
  phase = 'test' ;
end
...
if nargout > 0
  labels = imdb.imdb.images.label(batch) ;
  switch networkType
    case 'simplenn'
